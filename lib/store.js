...
module.exports = {
  ...
  setIfNotExists: async (attendeeId, record) => {
    ...
    records.push(record);
    await syncToCloud(records); // AWAIT IT SO VERCEL LAMBDA DOES NOT FREEZE IT BEFORE PUT FINISHES!
  ...
  updateRecordStatus: async (searchId, status, printedAt) => {
    ...
    if (rec) {
      ...
      await syncToCloud(records); // AWAIT IT SO VERCEL LAMBDA DOES NOT FREEZE IT!
      store.records = records;
      return rec;
    }
    return null;
