  ...
  try {
    if (fs.existsSync(STORE_FILE)) {
      ...
      if (Array.isArray(parsed)) {
        return parsed.filter(r => r && r.id && r.id !== 'CHK-1' && r.name);
      }
    }
  } catch (err) {
...
function writeDiskStore(records) {
  try {
    const valid = records.filter(r => r && r.id && r.id !== 'CHK-1' && r.name);
...
function mergeStores(localList, cloudList) {
  const map = new Map();
  for (const r of cloudList) {
    if (r && r.id && r.id !== 'CHK-1' && r.name) map.set(r.id, r);
  }
  for (const r of localList) {
    if (r && r.id && r.id !== 'CHK-1' && r.name) {
      ...
      if (!existing) {
        ...
        if (r.status === 'CHECKED_IN' || existing.status === 'CHECKED_IN') {
          map.set(r.id, {
            ...
            ...r,
            status: 'CHECKED_IN',
            printedAt: r.printedAt || existing.printedAt || new Date().toISOString()
          });
        } else {
...
async function syncFromCloud() {
  const local = readDiskStore();
  try {
    const res = await fetch(CLOUD_URL);
    if (res.ok) {
      const json = await res.json();
      if (json && json.data && Array.isArray(json.data.records)) {
        const merged = mergeStores(local, json.data.records);
        writeDiskStore(merged);
        if (global.__solsticeCheckInStoreInstance) {
          global.__solsticeCheckInStoreInstance.records = merged;
        }
        return merged;
      }
    }
...
module.exports = {
  ...
  getRecordByAttendeeId: async (attendeeId) => {
    let records = readDiskStore();
    let rec = records.find(r => r.attendeeId === attendeeId);
    if (!rec) {
      records = await syncFromCloud();
      rec = records.find(r => r.attendeeId === attendeeId);
    } else {
      syncFromCloud().catch(() => {});
    }
    return autoPromoteIfPending(rec, records);
  ...
  getRecord: async (id) => {
    let records = readDiskStore();
    let rec = records.find(r => r.id === id || r.attendeeId === id);
    if (!rec) {
      records = await syncFromCloud();
      rec = records.find(r => r.id === id || r.attendeeId === id);
    } else {
      syncFromCloud().catch(() => {});
    }
    return autoPromoteIfPending(rec, records);
  ...
  updateRecordStatus: async (searchId, status, printedAt) => {
    let records = readDiskStore();
    let rec = records.find(r => r.id === searchId || r.attendeeId === searchId);
    if (!rec) {
      records = await syncFromCloud();
      rec = records.find(r => r.id === searchId || r.attendeeId === searchId);
    }
    if (rec) {
      rec.status = status;
